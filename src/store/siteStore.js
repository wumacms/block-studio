...
export const useSiteStore = defineStore('site', {
                    ...
                    blocks: [
                        ...
                        },
                        {
                            ...
                            content: {
                                ...
                                content: '在 AntiGravity，我们不只是在编写代码或绘制图形。我们是在构建桥梁，连接 brand 与用户之间的情感纽带。通过深入的行业洞察和前沿的技术栈，我们确保您的每一个数字足迹都令人难忘。',
    ...
    getters: {
        activePage: (state) => {
            return state.siteData.pages.find(p => p.id === state.siteData.currentPageId) || state.siteData.pages[0]
        }
    },
    actions: {
        changePage(pageId) {
            this.siteData.currentPageId = pageId
        },
        toggleBlock(template) {
            const page = this.siteData.pages.find(p => p.id === this.siteData.currentPageId)
            ...
            if (index > -1) {
                ...
                page.blocks.push({
                    ...
                    template: template,
                    content: { ...this.blockDefaults[template] }
                })
            }
        }
    },
